...
const CHARM_CHECK_ACTION = {
  ...
  success: {
    narrative: { tokens: [{ text: 'They smile.', style: 'normal', speed: 'normal', pauseAfter: 0, effect: 'none', color: null }] },
    statChanges: null,
  ...
  failure: {
    narrative: { tokens: [{ text: 'They look away.', style: 'normal', speed: 'normal', pauseAfter: 0, effect: 'none', color: null }] },
    statChanges: null,
    statusChanges: { mood: -5 },
    moneyChange: null,
    itemsGained: null,
...
describe('getAvailableActions — real Kenton location data', () => {
  ...
  it('filters actions by stat requirements', () => {
    ...
    const eliteAction = { ...DRINK_ACTION, id: 'charm_action', requirements: { minStats: { charm: 50 } } };
    const result = getAvailableActions(player, location, makeGameTime(), [eliteAction]);
    expect(result).toHaveLength(0);
  });

  it('sorts available actions by descending weight', () => {
    const locationData = { ...kentonLocations.blue_parrot, actionIds: ['drink_pbr', 'chat_up_bartender'] };
    const location = createLocation(locationData);
    const player = createPlayer('Test');

    const result = getAvailableActions(player, location, makeGameTime(), [DRINK_ACTION, CHARM_CHECK_ACTION]);
...
describe('resolveAction — real Kenton action data', () => {
  it('raid_fridge auto-succeeds and returns hunger gain', () => {
    const player = createPlayer('Test');
    const result = resolveAction(player, kentonActions.raid_fridge, makeGameTime(), [], seededRandom(1));
  ...
  it('look_for_change critical success yields $20 on natural 20', () => {
    ...
    const result = resolveAction(player, kentonActions.look_for_change, makeGameTime(14), [], alwaysMax);
    expect(result.success).toBe(true);
  ...
  it('look_for_change critical failure on natural 1 — crit outcome selected regardless of success', () => {
    ...
    const result = resolveAction(player, kentonActions.look_for_change, makeGameTime(14), [], alwaysMin);
  ...
  it('shoplift_plaid blocked when sobriety below requirement', () => {
    ...
    const result = resolveAction(player, kentonActions.shoplift_plaid, makeGameTime(), [], seededRandom(1));
