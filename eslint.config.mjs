...
const config = defineConfig(
  // ── Canonical BFD preset (copied verbatim; the fence glob below is ours) ──
  {
...
// The preset's rules hold in component scripts too: the same rule set, for .vue.
const bfdRules = config.find((block) => block.rules?.['max-params']).rules
export default [
  ...config,
  { files: ['**/*.vue'], plugins: { '@typescript-eslint': tseslint.plugin }, rules: bfdRules },
  prettier,
];
